...
def set_rules(world, options: SM64Options, player: int, area_connections: dict, star_costs: dict, move_rando_bitvec: int):
    ...
    valid_move_randomizer_start_courses = [
        "Hazy Maze Cave", "Cool, Cool Mountain",
        "Big Boo's Haunt", "Lethal Lava Land", "Shifting Sand Land", "Snowman's Land"
    ]  # Excluding WF, BOB, WDW, TTM, THI, TTC, and RR
    ...
    connect_regions(world, player, "Bob omb battlefield: Goombas", randomized_entrances_s["Bob-omb Battlefield"])
    connect_regions(world, player, "Bob omb battlefield: Bombs", randomized_entrances_s["Bob-omb Battlefield"])
    connect_regions(world, player, "King Bob-omb", randomized_entrances_s["Bob-omb Battlefield"])
    ...
    connect_regions(world, player, "Jolly Roger Bay Door", randomized_entrances_s["Jolly Roger Bay"],
                    rf.build_rule("", painting_lvl_name="KRB"))
    ...
    rf.assign_rule("WF: Blast Away the Wall Wall broken", "CANN | CANNLESS & LG")
    # Jolly Roger Bay
    rf.assign_rule("JRB: Upper", "TJ/BF/SF/WK & SW | MOVELESS & LG $ SW")
    rf.assign_rule("JRB: Red Coins on the Ship Afloat", "CL/CANN/TJ & SW | MOVELESS & BF/WK & SW")
    rf.assign_rule("JRB: Blast to the Stone Pillar", "CANN+CL & SW")
    rf.assign_rule("JRB: Through the Jet Stream", "SW | CAPLESS")
    # Cool, Cool Mountain
    ...
    # Dire, Dire Docks
    rf.assign_rule("DDD: Pole-Jumping for Red Coins", "CL & SW & {{Bowser in the Fire Sea Key}} | TJ+DV+LG+WK & MOVELESS")
    rf.assign_rule("DDD: Through the Jet Stream", "SW | CAPLESS")
    rf.assign_rule("DDD: Collect the Caps...", "VC & SW | CAPLESS & VC")
    # Snowman's Land
    ...
    if options.enable_coin_stars:
        ...
        rf.assign_rule("SL: 100 Coins", "VC")
    ...
    # Castle Stars
    add_rule(world.get_location("Toad in Hazy Maze Cave Room (Star)", player), lambda state: state.can_reach("Basement", 'Region', player) and state.has("Power Star", player, 12) and state.has("Toad Soul (Toadcidious)", player))
    add_rule(world.get_location("Toad Across From Tall Tall Mountain Painting (Star)", player), lambda state: state.can_reach("Second Floor", 'Region', player) and state.has("Power Star", player, 25) and state.has("Toad Soul (Friendly DarkToad)", player))
    add_rule(world.get_location("Toad Next to Tick Tock Clock (Star)", player), lambda state: state.can_reach("Third Floor", 'Region', player) and state.has("Power Star", player, 35) and state.has("Toad Soul (OnlyLeafyToad)", player))
    add_rule(world.get_location("Toad in Whomps Fortress Room (Toady)", player), lambda state: state.has("Power Star", player, 1) and state.has("Toad Soul (Toady)", player))
    add_rule(world.get_location("Toad in Jolly Roger Bay Room (Toadlarone)", player), lambda state: state.has("Power Star", player, 3) and state.has("Toad Soul (Toadlarone)", player))
    add_rule(world.get_location("Toad in Basement Near Dead End (Toadphoon)", player), lambda state: state.can_reach("Basement", 'Region', player) and state.has("Toad Soul (Toadphoon)", player))
    add_rule(world.get_location("Toad in Hazy Maze Cave Room (NihiliToad)", player), lambda state: state.can_reach("Basement", 'Region', player) and state.has("Power Star", player, 12))
    add_rule(world.get_location("Toad Across From Tall Tall Moutain Painting (Friendly DarkToad)", player), lambda state: state.can_reach("Second Floor", 'Region', player) and state.has("Power Star", player, 25) and state.has("Toad Soul (Friendly DarkToad)", player))
    add_rule(world.get_location("Toad Next to Snowman Land Room (Darth Toad)", player), lambda state: state.can_reach("Second Floor", 'Region', player) and state.has("Toad Soul (Darth Toad)", player))
    add_rule(world.get_location("Toad Next to Tick Tock Clock (OnlyLeafyToad)", player), lambda state: state.can_reach("Third Floor", 'Region', player) and state.has("Power Star", player, 35) and state.has("Toad Soul (OnlyLeafyToad)", player))
    add_rule(world.get_location("BoB: Big Bob-Omb on the Summit", player), lambda state: state.has("Puncha", player) and state.has("Grab", player))
    add_rule(world.get_location("Challenge King Bob-omb", player), lambda state: state.has("Puncha", player) and state.has("Grab", player))
    add_rule(world.get_location("Throw King Bob-omb From the Mountain", player), lambda state: state.has("Puncha", player) and state.has("Grab", player))
    add_rule(world.get_location("Defeat King Bob-omb", player), lambda state: state.has("Puncha", player) and state.has("Grab", player))
